...
function initChat() {
  ...
  // Voice button - opens voice popup (temporarily using popup for reliability)
  voiceBtn.addEventListener('click', openVoicePopupDocked);
}

// ============================================================================
// Voice Assistant - Offscreen Document Integration
// ============================================================================

let offscreenDocumentReady = false;
let isVoiceDialogOpen = false;
...
async function setupOffscreenDocument() {
  ...
  try {
    await chrome.offscreen.createDocument({
      ...
      reasons: ['USER_MEDIA'],
      justification: 'Voice input for AI assistant'
    });
    console.log('✅ Offscreen document created');
    offscreenDocumentReady = true;

    // Initialize WebSocket in offscreen document
...
function openPermissionPopup() {
  ...
  console.log('🔐 Opening permission popup...');

  chrome.windows.create({
    url: chrome.runtime.getURL('permission.html?auto=true'),
    type: 'popup',
    width: width,
    height: height,
    ...
    const checkClosed = setInterval(() => {
      chrome.windows.get(window.id, () => {
        if (chrome.runtime.lastError) {
          ...
          console.log('🔐 Permission popup closed');

          // Check if permission was granted and open voice dialog
          checkMicrophonePermission().then(granted => {
            if (granted) {
              console.log('✅ Permission granted! Opening voice dialog...');
              openVoiceDialogAfterPermission();
            }
          });
        }
      });
...
// Show voice dialog (with permission check)
async function showVoiceDialog() {
  // Check if we have microphone permission
  const hasPermission = await checkMicrophonePermission();

  if (!hasPermission) {
    console.log('⚠️ Microphone permission not granted. Opening permission popup...');
    openPermissionPopup();
    return;
  }
...
// Open voice dialog (after permission granted)
async function openVoiceDialogAfterPermission() {
...
async function startVoiceListening() {
  ...
  if (!userEmail) {
    updateVoiceStatus('Error: Not logged in');
    return;
  }
  ...
  try {
    ...
    updateVoiceStatus('Error: Could not start listening');
  }
}
...
function updateVoiceTranscript(finalText, interimText) {
  ...
  if (!finalText && !interimText) {
    transcriptEl.innerHTML = '<div class="voice-transcript-placeholder">Your speech will appear here...</div>';
    return;
  }
  ...
  if (interimText) {
    html += `<div class="voice-transcript-interim">${interimText}</div>`;
  }
...
// Initialize voice dialog controls
function initVoiceDialog() {
  const voiceMicBtn = document.getElementById('voiceMicBtn');
  const closeVoiceBtn = document.getElementById('closeVoiceBtn');

  if (voiceMicBtn) {
    voiceMicBtn.addEventListener('click', toggleVoiceListening);
  }

  if (closeVoiceBtn) {
    closeVoiceBtn.addEventListener('click', hideVoiceDialog);
  }

  // Setup offscreen document on load
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.type) {
    ...
    case 'LISTENING_STOPPED':
      ...
      updateVoiceStatus('Sending in 1.5s...');
      break;

    case 'TRANSCRIPT_UPDATE':
      updateVoiceTranscript(message.final, message.interim);
      break;
    ...
    case 'AI_RESPONSE':
      ...
      updateVoiceStatus('Ready - Click mic for next command');
      break;
    ...
    case 'ERROR':
      ...
      updateVoiceStatus('Error - Click mic to try again');
      isVoiceListening = false;
      updateVoiceMicButton(false);
      break;
    ...
    case 'MICROPHONE_READY':
      console.log('✅ Microphone ready in offscreen document');
      break;
...
// ============================================================================
// Voice Interface - Docked Popup Window (Reliable Solution)
// ============================================================================

// Open voice popup in a docked position (more reliable than offscreen)
function openVoicePopupDocked() {
  const width = 420;
  const height = 600;
  ...
  chrome.windows.create({
    ...
    console.log('✅ Voice popup opened:', window.id);
  });
}

// ============================================================================
// Voice Interface - Alternative Offscreen Approach (Has Permission Issues)
// ============================================================================
// Note: The offscreen API approach below has microphone permission issues
// Using popup window instead for now (see openVoicePopupDocked above)
